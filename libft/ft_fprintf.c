...
/*   Updated: 2025/01/23 15:34:14 by oriabenk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	printlist(int fd, const char *format, va_list args)
{
	ssize_t	counter;
	...
	while (*format)
	{
		if (*format == '%')
		{
			...
			if (!*format)
				return ((int)counter);
			counter += print_arguments(fd, *format, args);
			format++;
		}
		else
		{
...
int	ft_fprintf(int fd, const char *format, ...)
{
	...
	quantuty_character = printlist(fd, format, args);
	va_end(args);
